...
- (void)layoutSubviews
{
	...
	if (!_loaded) {
		...
		if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ) {
			...
			if (_showTitleLabel) {
				titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width - 20, 24)];
				[titleLabel setCenter:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2 + 15)];
